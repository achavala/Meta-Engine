...
def generate_report(calls: List[Dict], puts: List[Dict], data: Dict) -> str:
    """Generate the full markdown analysis report."""
    now = datetime.now(EST)
    report = []
    report.append(f"# 🎯 3 PM EST INSTITUTIONAL OPTIONS ANALYSIS")
    ...
    report.append("")
    report.append(f"*Generated by Meta Engine 3PM Analysis | {now.strftime('%b %d %Y %I:%M %p ET')}*")
...
def send_email(report_md: str, calls: List[Dict], puts: List[Dict]) -> bool:
    ...
    msg["Subject"] = (
        f"🎯 3PM OPTIONS: 🟢 CALLS {call_syms} | 🔴 PUTS {put_syms} — "
...
def send_telegram(calls: List[Dict], puts: List[Dict]) -> bool:
...
def run_3pm_analysis():
    """Full 3 PM analysis pipeline."""
    now = datetime.now(EST)
    logger.info("=" * 70)
    logger.info(f"  🎯 3 PM EST INSTITUTIONAL OPTIONS ANALYSIS")
    ...
    report = generate_report(calls, puts, data)

    # Save report
    out_dir = Path("output")
    out_dir.mkdir(exist_ok=True)
    report_path = out_dir / f"3pm_analysis_{now.strftime('%Y%m%d')}.md"
    ...
    email_ok = send_email(report, calls, puts)

    # 5. Send Telegram
    logger.info("\n📱 STEP 5: Sending Telegram...")
    tg_ok = send_telegram(calls, puts)
